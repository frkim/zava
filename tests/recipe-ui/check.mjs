...
await wait(text('Produits non sélectionnés (1)'), 'deselected cartouche opens on removal');
...
assert.equal(await evaluate(text('vous les avez peut-être déjà chez vous')), true);
...
assert.equal(await evaluate(text('16,98')), true);
console.log('PASS deselecting moves a product into the collapsible cartouche, updates the total, and re-selecting restores it');
